...
export class FilterByCategoryComponent implements OnInit, OnDestroy {
  ...
  getBrandsByCategory(id: number): void {
    const productInCat = MOCK_PRODUCTS.filter(p => p.category_id === id);
  ...
  getCategoryName(categoryId: number): void {
    const category = MOCK_CATEGORIES.find(c => c.category_id === categoryId);
    this.categoryName = category ? category.category_name : null;
  }

  filterByPriceCategory(categoryId: number, priceCategory: string): void {
    let products = MOCK_PRODUCTS.filter(p => p.category_id === categoryId);
    ...
    this.currentPage = 1;
    this.updatePagination();
    this.noProductsMessage = this.DsSP.length === 0 ? 'Không có sản phẩm nào phù hợp.' : '';
  }
